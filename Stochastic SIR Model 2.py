...
def main():
    ...
    for k in range(0,3):
        for m in range(0,3):    
            ...
            name='Project3_probSus_'+str(probSus_vector[k])+'probInf_'+str(probInf_vector[m])
            ...
            plt.xlabel('Days')
            plt.ylabel('Population')
            plt.title('Stochastic model 2 (20 days) \n probSus:'+str(probSus_vector[k])+'; probInf:'+str(probInf_vector[m]))
            plt.legend(fontsize=8)
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                if self.matrix[i][j] == 0: 
                    self.numSickNeighbor=0
                    for k in range(len(self.neighbors)):                        
                        if self.neighbors[k] in self.infected:
                            self.numSickNeighbor +=1
                    if  self.numSickNeighbor>0:
                        self.probCatch=float(self.numSickNeighbor)/float(len(self.neighbors))
                        self.pro1=[self.probCatch,1]
                        self.new_matrix[i][j] =  probability.ProbCatchF(self.value1,self.pro1)
