...
def main():
    ...
    for k in range(0,3):
        for m in range(0,3):    
            ...
            name='Project 4 probSus_'+str(probSus_vector[k])+'probInf_'+str(probInf_vector[m])            
            ...
            plt.xlabel('Days')
            plt.ylabel('Population')
            plt.title('Stochastic model 3 (20 days)\n probSus:'+str(probSus_vector[k])+'; probInf:'+str(probInf_vector[m]))
            plt.legend(fontsize=8)
            ...
            for j in range(self.width):
                ...
                if self.matrix[i][j] == 0: 
                    ...
                    for k in range(len(self.neighbors)):                        
                        if self.neighbors[k] in self.infected1:
                            self.level += 1.0
                        elif  self.neighbors[k] in self.infected2:
                            self.level += 0.5   
                    if  self.level > 0.0:
                        self.probCatch=float(self.level)/len(self.neighbors)
